...
class _AbaCheckoutScreenState extends State<AbaCheckoutScreen> with WidgetsBindingObserver {
  ...
  bool _isInitiating = false; // Add this
  String? _amount;
  String _selectedMethod = 'abapay_khqr';
  ...
  void _navigateToAbaCheckout(String abaOption, String methodName) async {
    ...
    // ── COF (card link/payment) flow ────────────────────────────────────────
    // ABA's COF endpoint requires multipart/form-data, so we let the WebView
    // handle the POST via an auto-submitting HTML form (no intermediate http.post).
    if (abaOption == 'cards') {
      _openWebView(paywayPayload, paywayApiUrl, methodName);
      return;
    }

    // ── KHQR / ABA deeplink flow ────────────────────────────────────────────
    // These endpoints return JSON (QR image / deeplink), so we POST first and
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (isJson) {
          ...
          if (jsonResponse['status']['code'] == '00') {
            ...
            if (paymentUrl != null && paymentUrl.isNotEmpty) {
              if (!paymentUrl.startsWith('http')) {
                ...
                try {
                  bool launched = await launchUrl(Uri.parse(paymentUrl), mode: LaunchMode.externalApplication);
                  if (!launched) throw Exception('Could not launch the ABA Mobile app.');
                  setState(() => _isWaitingForReturn = true);
                } catch (e) {
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
          ...
          // HTML response — load in WebView directly
      ...
      if (errorMsg.contains('Response:')) {
        showDialog(
          ...
          builder: (context) => AlertDialog(
            ...
            content: SingleChildScrollView(child: SelectableText(errorMsg)),
            actions: [
              TextButton(
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildPaymentMethodTile(
                        ...
                        isSelected: _selectedMethod == 'abapay_khqr',
                        onTap: () => setState(() => _selectedMethod = 'abapay_khqr'),
                      ),
        ...
        if (orderProvider.isLoading || _isCheckingPayment || _isInitiating)
          Container(
            ...
            child: const Center(
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 16),
                  Text('Initiating payment...', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                ],
              ),
            ),
          ),
          ...
          children: [
            Container(
              ...
              child: SvgPicture.asset(
                svgAsset,
                width: svgAsset.contains('khqr') ? 44 : 36,
                fit: BoxFit.contain,
              ),
            ),
                ...
                children: [
                  Text(
                    ...
                    style: TextStyle(
                      fontWeight: FontWeight.bold,
                      fontSize: 16,
                      color: isSelected ? AppColors.primaryStart : AppColors.textPrimaryLight,
                    ),
                  ),
                  const SizedBox(height: 4),
  ...
  Widget _buildSavedCardTile(SavedCard card) {
    ...
    final isSelected = _selectedMethod == 'card_${card.index}';

    return Container(
      margin: const EdgeInsets.only(bottom: 12),
      child: InkWell(
        onTap: () => setState(() => _selectedMethod = 'card_${card.index}'),
        borderRadius: BorderRadius.circular(16),
        child: AnimatedContainer(
          duration: const Duration(milliseconds: 300),
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [
              BoxShadow(
                color: isSelected 
                    ? (isVisa ? const Color(0xFF1A1F71) : AppColors.primaryStart).withOpacity(0.4)
  ...
  Widget _buildActionButton(ThemeData theme) {
    ...
    if (_selectedMethod.startsWith('card_')) buttonText = 'Pay Now with Card';
    if (_selectedMethod == 'add_card') buttonText = 'Continue to Add Card';
    ...
    return Container(
      ...
      child: ElevatedButton(
        onPressed: _isInitiating ? null : _handlePaymentProceed,
  ...
  Future<void> _handlePaymentProceed() async {
    if (_selectedMethod == 'abapay_khqr' || _selectedMethod == 'abapay_deeplink') {
      _navigateToAbaCheckout(_selectedMethod, _selectedMethod == 'abapay_khqr' ? 'ABA KHQR' : 'ABA Mobile App');
    } else if (_selectedMethod.startsWith('card_')) {
      final index = int.parse(_selectedMethod.split('_')[1]);
      final card = _savedCards.firstWhere((c) => c.index == index);
      _payByToken(card);
    } else if (_selectedMethod == 'add_card') {
