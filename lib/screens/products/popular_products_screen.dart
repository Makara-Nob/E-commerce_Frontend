...
class _PopularProductsScreenState extends State<PopularProductsScreen> {
  ...
  Future<void> _loadMore({bool refresh = false}) async {
    ...
    try {
      final ApiResponse<ProductListResponse> res =
          await _productService.getPopularProducts(page: _page, limit: _limit);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Popular Products', style: TextStyle(fontWeight: FontWeight.bold)),
                        ...
                        if (!_hasMore && _products.isNotEmpty)
                          SliverToBoxAdapter(
                            child: Padding(
                              ...
                              child: Center(
                                child: Text(
                                  "You've seen all ${_products.length} popular products!",
